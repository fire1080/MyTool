...
#region HotKey
        ...
        public class HotKey
        {
            ...
            public DateTime LastTriggerTime = DateTime.Now;
        ...
        //注意：不应在HookProc中使用ShowDialoge()等阻断线程的语句，会导致速度奇慢。
        //      不应多次SetWindowsHookEx()同一段HookProc，键盘会混乱。
        //      HookProc中的键盘状态是停滞的，重量级操作应在子线程中执行。
                ...
                if (_hotKeyCodes.Contains((Keys) keyboardHookStruct.vkCode))
                {
                    ...
                    var matchedHotKeys =
                        _hotKeyDic.Values.Where(
                            h =>
                                h.Control == control && h.Alt == alt && h.Shift == shift && h.Win == win && 
                    ...
                    foreach (var hotKey in matchedHotKeys)
                    {
                        ...
                        switch (wParam)
                        {
                            case WM_KEYDOWN:
                            case WM_SYSKEYDOWN:
                                if (TimeSpan.FromMilliseconds(hotKey.Interval) > DateTime.Now.Subtract(hotKey.LastTriggerTime))
                                    continue;
                                ...
                                hotKey.LastTriggerTime = DateTime.Now;
                                break;
                            case WM_KEYUP:
                            case WM_SYSKEYUP:
                        ...
                        Debug.Print("hotkey Id={0},code={1}", hotKey.Id, hotKey.KeyCode);
                    }
                }
            }
